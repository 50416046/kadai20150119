...
public class MySQL {
   ...
   String server, dbname, url, user, password;
    ...
    public MySQL() {
        ...
        this.dbname  = "50416046";         // データベース名;

        this.url = "jdbc:mysql://" + server + "/" + dbname + "?useUnicode=true&characterEncoding=UTF-8";

        this.user = "50416046";         // データベース作成ユーザ名;

        this.password  = "50416046";     // データベース作成ユーザパスワード;
...
public int OutReservation(String rdate, String st, String et, String reservation_userid, String facility) {
	int rs_int = 0;
	String sql = "DELETE FROM reservation WHERE date='" +rdate +"' AND start_time='" + st + "' AND end_time='" + et + "'And facility_name='"+ facility+ "';";
	try {
		rs_int = stmt.executeUpdate(sql);
	} catch (SQLException e) {
		// TODO Auto-generated catch block
...
public ResultSet selectExp(String facility){
